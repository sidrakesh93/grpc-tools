 ...
 * It ensures that
 * - the binaries needed to complete code generation are ready available
 ...
 */
ApiRepo.prototype._findProtos = function _findProtos(name, version, done, onProto) {
...
ApiRepo.prototype._makeProtocFunc = function _makeProtocFunc(opts, language) {
  ...
  var callProtoc = function callProtoc(protoPath, done) {
    ...
    if (language === 'go') {
      ...
      var whichCmd = 'which grpc_' + language + '_plugin';
      args.push(
        '--' + language + '_out=' + outDir,
        '--grpc_out=' + outDir,
        '--plugin=protoc-gen-grpc=`' + whichCmd + '`',
        protoPath
      );
    }
